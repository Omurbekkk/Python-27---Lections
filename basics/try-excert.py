...
SyntaxError
# Исключения, которое выходит если допущена синтаксическая ошибка
'''
(
...
TypeError
# Исключение, которое выходит когда мы что то не свойственное данному типу данных
...
# Except   может быть несколько
    ...
    # код, который отработает только если в блоке try ни одна ошибка не вызывалась
    print('Введенное число: ', num)
finally:
    # код, который отрабатывает вообще в любом случае
    # хоть вызвалась ошибка, хоть нет
    print('До свидания')
